...
public class PlayerViewModelSim {
    ...
    public void Load(string iframeUrl, int? kpId, int? season, int? episode, string selectedVoiceover) {
        BeginLoad(iframeUrl, kpId, season, episode, selectedVoiceover);
    }

    public void BeginLoad(string iframeUrl, int? kpId, int? season, int? episode, string selectedVoiceover) {
        CurrentKpId = kpId;
        ...
        if (kpId.HasValue && !string.IsNullOrEmpty(selectedVoiceover)) {
    ...
    public void PlayEpisode(Tuple<int, int, AllohaTranslation> ep) {
        ...
        BeginLoad(ep.Item3.IframeUrl, CurrentKpId, ep.Item1, ep.Item2, ep.Item3.Name);
    }
}
...
public class TestRunner {
    public static void RunAllTests() {
        ...
        // Test 1: Single translation movie
        var movie1Trans = new List<AllohaTranslation>();
        movie1Trans.Add(new AllohaTranslation("1", "\u0414\u0443\u0431\u043B\u044F\u0436", "https://stream.alloha.tv/1"));
        var movie1 = new AllohaMovie("\u0418\u043D\u0442\u0435\u0440\u0441\u0442\u0435\u043B\u043B\u0430\u0440", "https://stream.alloha.tv/1", movie1Trans);
        var apiResult1 = new AllohaApiResult();
        apiResult1.Title = "\u0418\u043D\u0442\u0435\u0440\u0441\u0442\u0435\u043B\u043B\u0430\u0440";
        apiResult1.IsSerial = false;
        apiResult1.Movie = movie1;

        var vm1 = new PlayerViewModelSim();
        vm1.SeriesResult = apiResult1;
        vm1.Load("https://stream.alloha.tv/1", 258687, null, null, "\u0414\u0443\u0431\u043B\u044F\u0436");

        if (vm1.AvailableVoiceovers.Count == 1 && vm1.AvailableVoiceovers[0] == "\u0414\u0443\u0431\u043B\u044F\u0436" && vm1.CurrentTranslationName == "\u0414\u0443\u0431\u043B\u044F\u0436") {
            Console.WriteLine("[PASS] Test 1: Single translation movie initialized correctly.");
        } else {
            Console.WriteLine("[FAIL] Test 1: Single translation movie initialization failed!");
        }

        // Test 2: Movie with 15+ translations
        var movie15Trans = new List<AllohaTranslation>();
        string[] voices = new string[] {
            "\u0414\u0443\u0431\u043B\u044F\u0436", "Red Head Sound", "LostFilm", "HDRezka",
            "\u041A\u0443\u0431\u0438\u043A \u0432 \u041A\u0443\u0431\u0435", "\u041F\u0438\u0444\u0430\u0433\u043E\u0440",
            "Flarrow Films", "TVShows", "NewStudio", "ColdFilm",
            "\u041A\u0443\u0440\u0430\u0436-\u0411\u0430\u043C\u0431\u0435\u0439", "Jaskier", "AlexFilm", "RuDub", "Original", "\u0421\u0443\u0431\u0442\u0438\u0442\u0440\u044B"
        };
        int idx = 1;
        foreach (var v in voices) {
            movie15Trans.Add(new AllohaTranslation(idx.ToString(), v, "https://stream.alloha.tv/" + idx));
            idx++;
        }
        var movie15 = new AllohaMovie("\u0414\u044D\u0434\u043F\u0443\u043B \u0438 \u0420\u043E\u0441\u043E\u043C\u0430\u0445\u0430", "https://stream.alloha.tv/1", movie15Trans);
        var apiResult15 = new AllohaApiResult();
        apiResult15.Title = "\u0414\u044D\u0434\u043F\u0443\u043B \u0438 \u0420\u043E\u0441\u043E\u043C\u0430\u0445\u0430";
        apiResult15.IsSerial = false;
        apiResult15.Movie = movie15;

        var vm15 = new PlayerViewModelSim();
        vm15.SeriesResult = apiResult15;
        vm15.Load("https://stream.alloha.tv/1", 123456, null, null, "\u0414\u0443\u0431\u043B\u044F\u0436");

        if (vm15.AvailableVoiceovers.Count == 16) {
            Console.WriteLine("[PASS] Test 2: Movie with 15+ translations preserves all options in availableVoiceovers.");
        } else {
            Console.WriteLine("[FAIL] Test 2: Movie translations count mismatch: " + vm15.AvailableVoiceovers.Count);
        }

        // Test 3: In-player voiceover switching with savedTime preservation
        vm15.CurrentTime = 1450.5; // 24 mins in
        vm15.SwitchVoiceover("Red Head Sound", 1);

        if (vm15.CurrentTranslationName == "Red Head Sound" && vm15.CurrentIframeUrl == "https://stream.alloha.tv/2" && Math.Abs(vm15.CurrentTime - 1450.5) < 0.001) {
            Console.WriteLine("[PASS] Test 3: In-player voiceover switching preserves playback position (" + vm15.CurrentTime + "s) and updates iframe URL.");
        } else {
            Console.WriteLine("[FAIL] Test 3: Voiceover switch failed. Translation: " + vm15.CurrentTranslationName + ", Time: " + vm15.CurrentTime + ", Url: " + vm15.CurrentIframeUrl);
        }

        // Test 4: Series Episode Transition
        // Ep 1: Dubbed, LostFilm, HDRezka
        // Ep 2: LostFilm only (Dubbed missing)
        // Ep 3: Dubbed, LostFilm, HDRezka
