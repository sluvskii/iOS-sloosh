...
object CollapsDashRewriter {
    private const val TAG = "CollapsDashRewriter"
    ...
    fun rewrite(
        ...
        if (manifest.isBlank()) return manifest
        Log.d(TAG, "rewrite: mediaId=$mediaId voices=${voices.size} subtitles=${subtitles.size}")
        if (voices.isNotEmpty()) {
            Log.d(TAG, "rewrite: voices=${voices.joinToString(" | ")}")
        }
        ...
        var audioSetSequentialIndex = 0
        val rewrittenAudio = audioSetRegex.replace(manifest) { m ->
            val block = m.value
            ...
            val idxFromLang = rawLang
                ?.let { Regex("^ru(\\d+)$", RegexOption.IGNORE_CASE).find(it) }
                ?.groupValues
                ?.getOrNull(1)
                ?.toIntOrNull()
            val idx = idxFromLang ?: audioSetSequentialIndex
            audioSetSequentialIndex += 1

            val voiceName = voices.getOrNull(idx)
            val fallbackLabel = "Audio ${idx + 1}"
            Log.d(TAG, "audioSet[$idx]: rawLang=$rawLang voiceName=$voiceName openTag=$openTag")
        ...
        Log.d(TAG, "rewrite: matchedAudioSets=$audioSetSequentialIndex")
    ...
    private fun buildSubtitlesAdaptationSets(subtitles: List<CollapsSubtitle>): String {
